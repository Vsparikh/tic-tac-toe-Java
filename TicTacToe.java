...
public class TicTacToe {
	...
	public static int[] makePerfectMove() {
		...
		if (comWin[0] == 1) {
			...
			if (board[1][1] == '_') {
				pos[0] = 1;
				pos[1] = 1;
			} else if (board[0][0] == 'x'&& board[2][2] == '_') {
				pos[0] = 2;
				pos[1] = 2;
	...
	public static void displayResult(char symbol) {
		...
		c.setColor(new Color(0, 128, 128));
		if (symbol == 'x') {
			c.drawString("X Wins", 450, 320);
		} else if (symbol == '0') {
			c.drawString("O Wins", 450, 320);
		} else {
			c.drawString("Tie Game", 450, 320);
		}
		myFont = new Font("Comic Sans Ms", Font.BOLD, 50);
		c.setFont(myFont);
	...
	public static boolean playAgain() {
		...
		c.setFont(myFont);
		c.setColor(new Color(0, 128, 128));
		c.drawString("Would you like to play Again ? ", 300, 300);
		c.drawString("Enter 'Y' for yes or 'N' for no", 280, 430);
		int turn = c.getChar();
